...
namespace Mihcelle.Hwavmvid.Modules.ChatHubs.Services
{
    ...
    public class ChatHubService : IDisposable
    {
        ...
        public void BuildHubConnection(string username, string moduleId)
        {
            ...
            var accesstoken = this.Cookiesprovider.Getcookie(Shared.Constants.Authentication.Authcookiename);
        ...
        public async Task OnDataAvailableEventHandlerExecute(string dataUri, string id1, string id2)
        {
            try
            {
                if (this.Connection?.State == HubConnectionState.Connected)
                {
                    ...
                    if (room != null)
                    {
                        await this.Connection.SendAsync("UploadDataUri", dataUri, this.ModuleId, id1, id2, room.Motiondetection, room.Motiondetectionfluctation).ContinueWith((task) =>
                        {
                            if (task.Status == TaskStatus.RanToCompletion || task.Status == TaskStatus.Faulted)
                            {
                                if (this.TryHandleException(task))
        ...
        {
            await this.Connection.InvokeAsync("UploadSnapshotUri", imageUri, roomId, snapshotActivatorType).ContinueWith((task) =>
            {
                if (task.Status == TaskStatus.RanToCompletion || task.Status == TaskStatus.Faulted)
                {
                    if (this.TryHandleException(task))
        ...
        {
            await this.Connection.InvokeAsync("StartCam", this.ModuleId, model.Id1, model.Id2).ContinueWith((task) =>
            {
                if (task.Status == TaskStatus.RanToCompletion || task.Status == TaskStatus.Faulted)
                {
                    if (this.TryHandleException(task))
        ...
        {
            await this.Connection.InvokeAsync("StopCam", model.Id1, model.Id2).ContinueWith((task) =>
            {
                if (task.Status == TaskStatus.RanToCompletion || task.Status == TaskStatus.Faulted)
                {
                    if (this.TryHandleException(task))
        ...
        public async Task<ChatHubDevice> GetDefaultDevice(string roomId, ChatHubDeviceType type)
        {
            ...
            if (this.Connected)
            {
                await this.Connection.InvokeAsync<ChatHubDevice>("GetDefaultDevice", roomId, type).ContinueWith((task) =>
                {
                    if (task.Status == TaskStatus.RanToCompletion || task.Status == TaskStatus.Faulted)
                    {
                        if (this.TryHandleException(task))
        ...
        {
            await this.Connection.InvokeAsync("SetDefaultDevice", e.Id, e.Item.id, e.Item.name, ChatHubDeviceType.Audio).ContinueWith((task) =>
            {
                if (task.Status == TaskStatus.RanToCompletion || task.Status == TaskStatus.Faulted)
                {
                    if (this.TryHandleException(task))
        ...
        {
            await this.Connection.InvokeAsync("SetDefaultDevice", e.Id, e.Item.id, e.Item.name, ChatHubDeviceType.Microphone).ContinueWith((task) =>
            {
                if (task.Status == TaskStatus.RanToCompletion || task.Status == TaskStatus.Faulted)
                {
                    if (this.TryHandleException(task))
        ...
        {
            await this.Connection.InvokeAsync("SetDefaultDevice", e.Id, e.Item.id, e.Item.name, ChatHubDeviceType.Webcam).ContinueWith((task) =>
            {
                if (task.Status == TaskStatus.RanToCompletion || task.Status == TaskStatus.Faulted)
                {
                    if (this.TryHandleException(task))
        ...
        {
            await this.Connection.InvokeAsync<VideoPlayerApiItem>("DowloadDataUri", this.ModuleId, obj.ParameterId1, obj.ParameterId2, obj.LastSequenceId, obj.SliderCurrentValue, obj.SliderValueChanged).ContinueWith(async (task) =>
            {
                if (task.Status == TaskStatus.RanToCompletion || task.Status == TaskStatus.Faulted)
                {
                    if (this.TryHandleException(task))
        ...
        public async Task DownloadVideo(string moduleId, string connectionId, int roomId, int camId)
        {
            ...
            var apiUri = string.Concat("api/chathub", "/DownloadVideo");

        ...
        public async Task<DownloadApiItem> DownloadVideoCapture(string connectionId, string camId, string roomId, string moduleId, string fileType)
        {
            try
            {
                ...
                if (contextUser != null)
                {
                    ...
                    GlobalFFOptions.Configure(new FFOptions { BinaryFolder = "./wwwroot/" + ChatHubConstants.ModulePath, TemporaryFilesFolder = "/tmp" });
                    ...
                    if (contextCam != null)
                    {
                        ...
                        if (camSequences != null)
                        {
                            ...
                            string webRootPath = string.Concat(this.webHostEnvironment.ContentRootPath, "\\wwwroot");
                            string tempDirectoryPath = Path.Combine(webRootPath, "modu", "mhcc", "temp");
                            string videosDirecotryPath = Path.Combine(webRootPath, "modu", "mhcc", "videos", contextUser.UserName, moduleId, roomId, camId);
                ...
                if (contextUser != null)
                {
                    ...
                    if (contextCam != null)
                    {
                        ...
                        if (camSequences != null)
                        {
                            ...
                            string tempDirectoryPath = Path.Combine(tempWebRootPath, "modu", "mhcc", "temp");
                            string tempFullPath = Path.Combine(tempDirectoryPath, tempFilename + tempFileExtension);
                            ...
                            foreach (var camSequence in camSequences.Select((item, index) => new { item = item, index = index }))
                            {
                                ...
                                string filename = camSequence.item.Filename;
                                string webRootPath = string.Concat(this.webHostEnvironment.ContentRootPath, "\\wwwroot");
                                string directoryPath = Path.Combine(webRootPath, "modu", "mhcc", "videos", contextUser.UserName, moduleId, roomId, camId);
